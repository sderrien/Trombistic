...
class PhotoLabel extends JLabel {
	...
	def void setImage(String filename) {
		if (new File(filename).exists()) {
			// Add your image here
			photo= filename
			// Rect face =  FaceDetection.detectFace(filename);
			...
			System::err.println('''«filename» does not exist'''.toString)
		}
	}
	...
	override void paint(Graphics g) {
		...
		if (boundaries!==null) {
			...
			g.drawRect(boundaries.topLeft.x.intValue, boundaries.topLeft.y.intValue, boundaries.width.intValue, boundaries.height.intValue)
		}
	}
}
